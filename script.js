...
(function () {
  ...
  /* Fallback path: use the device's native camera app via a file input.
     This works even when the browser's live webcam API (getUserMedia) is
     blocked, unsupported, or misbehaving — very reliable on iPad Safari. */
  const fallbackInput = document.getElementById("camera-fallback-input");

  function openFallbackCamera() {
    fallbackInput.value = ""; // allow picking the same file twice in a row
    fallbackInput.click();
  }

  document.getElementById("btn-camera-fallback").addEventListener("click", openFallbackCamera);
  document.getElementById("btn-camera-fallback-link").addEventListener("click", openFallbackCamera);
  ...
  fallbackInput.addEventListener("change", () => {
    ...
    reader.onload = () => {
      addPhotoFromDataUrl(reader.result);
    };
    reader.readAsDataURL(file);
  });

  // Shared finish-a-photo logic used by both the live capture and the
  // fallback file-upload path, so behavior stays identical either way.
